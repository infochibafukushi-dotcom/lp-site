(function(){
  ...
  function renderMenuList(section){
    ...
    const navHtml = groups.length ? `
      <div style="position:sticky;top:72px;z-index:25;background:${window.IndexUtils.escapeAttr(section.bgColor || "#f7f5f0")};padding:0 20px 12px 20px;">
        <div style="max-width:720px;margin:0 auto;overflow-x:auto;-webkit-overflow-scrolling:touch;">
          <div style="display:flex;align-items:center;gap:12px;min-width:max-content;padding-top:4px;">
            ${groups.map((group, idx) => `
              <button
                type="button"
                style="display:inline-flex;align-items:center;justify-content:center;white-space:nowrap;padding:8px 16px;border:none;border-radius:999px;background:#2d2a26;color:#f0ebe0;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 1px 2px rgba(0,0,0,.08);"
                onclick="(function(){var el=document.getElementById('menu-list-cat-${idx}');if(el){el.scrollIntoView({behavior:'smooth',block:'start'});}})()"
              >${window.IndexUtils.escapeHtml(group.title || `カテゴリ${idx + 1}`)}</button>
            `).join("")}
          </div>
        </div>
      </div>
    ...
    const bottomCardHtml = bottomCard.visible ? `
      <div style="margin:6px 0 0 0;background:linear-gradient(180deg,#fffaf2 0%,#fff6ea 100%);border:1px solid #eadfce;border-radius:16px;padding:18px 18px 20px 18px;box-shadow:0 2px 8px rgba(0,0,0,.03);">
        ...
        ${bottomCard.buttonText ? `<div style="margin-top:14px;"><a href="${window.IndexUtils.escapeAttr(bottomCard.buttonUrl || '#')}" style="display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:0 16px;border-radius:999px;background:#2d2a26;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;">${window.IndexUtils.escapeHtml(bottomCard.buttonText || '')}</a></div>` : ''}
      </div>
    ` : '';
