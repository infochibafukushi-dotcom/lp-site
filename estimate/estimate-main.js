(function(){
  ...
  async function ensureEstimateNumber(result){
    ...
    if(window.EstimateStore?.createEstimateRecord){
      const issued = await window.EstimateStore.createEstimateRecord(state.storeId, state.config, payload);
      state.estimateNumber = issued.estimateNumber;
      state.estimateCreatedAt = issued.createdAt;
    }else if(window.EstimateNumber){
  ...
  async function init(){
    ...
    try{
      if(!window.EstimateStore || !window.EstimateStore.isEnabled()){
        throw new Error("Firebase が未設定です。管理者に shared/firebase-config.js の設定を依頼してください。");
      }
      await loadCtaUrls();

      const urlState = window.EstimateUrl?.parseUrlState?.() || {};
      const storeId = urlState.storeId || window.EstimateStore.getStoreId();
      state.storeId = storeId;
      state.config = await window.EstimateStore.loadEstimateConfig(storeId);
