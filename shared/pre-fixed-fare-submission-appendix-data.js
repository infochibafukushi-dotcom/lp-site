(function(global){
  ...
  function distanceFareFields(meta){
    ...
    return [
      ...
      ["深夜早朝割増", "22時から翌5時まで2割増"],
      ["障害者割引", "身体障害者・知的障害者は1割引"],
      ["端数処理", "事前確定運賃算定時は1円の位を四捨五入し10円単位とする（例: 4,861.6円→4,860円）"],
      ["適用開始予定日", "認可日以降、令和8年9月1日予定"],
      ["備考", ""]
    ];
  }
  ...
  function applicationHelperFields(meta){
    return [
      ...
      ["算定式", "事前確定運賃 ＝ 距離制運賃 × 平準化係数（1円の位を四捨五入し10円単位）"],
