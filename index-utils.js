(function(){
  ...
  function buildPcTopPhoneFallback(config){
    ...
    const defaultNumber = "090-6331-4289";
  ...
  function ensurePopupButtonShape(button, defaultText){
    return {
  ...
  function ensurePopupSettingsShape(config){
    const raw = config && config.popupSettings && typeof config.popupSettings === "object"
      ? config.popupSettings
      : {};
    const patterns = Array.isArray(raw.patterns) ? raw.patterns.slice(0, 3) : [];
    while(patterns.length < 3){
      patterns.push({});
    }
    return {
      patterns: patterns.map((pattern, index) => ensurePopupPatternShape(pattern, index))
    };
  }
